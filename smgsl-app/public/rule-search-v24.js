...
(function(){
  ...
  function norm(value){return String(value||'').toLowerCase().replace(/10\s*&?\s*under|10u/g,'10u').replace(/12\s*&?\s*under|12u/g,'12u').replace(/8\s*&?\s*under|8u/g,'8u').replace(/6\s*&?\s*under|6u/g,'6u').replace(/mixed division|mixed/g,'mx').replace(/3rd/g,'third').replace(/innings?/g,'inning').replace(/pitchers?/g,'pitcher').replace(/pitched|pitching/g,'pitch').replace(/stealing/g,'steal').replace(/walks/g,'walk').replace(/runs/g,'run').replace(/[^a-z0-9]+/g,' ').trim();}
  ...
  function tokens(query){return [...new Set(expanded(query).split(/\s+/).filter(t=>t.length>1&&!STOP.has(t)))];}
  ...
  function scorePage(row,query,division,source){
    ...
    const words=new Set(row.hay.split(/\s+/)),preferred=row.source==='USA Softball 2026'&&row.page===preferredPage(query);
  ...
  function pageCard(r,query){
    const label=r.source==='SMGSL Rules'?'SMGSL official rulebook':'USA Softball 2026 official rulebook';
  ...
  function renderResults(out,query,division,source){
    ...
    if(!quick.length&&!pages.length){out.innerHTML=`<div class="empty"><b>No exact ${esc(LABELS[division])} rule match for “${esc(value)}”.</b><br>Try a shorter term or a common call such as obstruction, HBP, look back, interference, pitching, stealing, or time limit.</div>`;return;}
