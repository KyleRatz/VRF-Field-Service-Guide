...
(function(){
  ...
  function pluralDay(day){if(!day||day==='Unknown day')return day;return day.endsWith('s')?day:day+'s';}
  function seasonFor(v){
    const d=typeof safeFieldDate==='function'?safeFieldDate(v):new Date(v);
    if(!d||Number.isNaN(d.getTime()))return 'Season not identified';
  ...
  function practicePattern(x){
    ...
    const day=siWeekday(x&&x.start),season=seasonFor(x&&x.start);
    ...
    return {field,day,season,hrs,start,end,key:[season,field,day,start,end,hrs.toFixed(2)].join('|')};
  }
  function scheduleHtml(patterns){
    ...
    const order={Sunday:0,Monday:1,Tuesday:2,Wednesday:3,Thursday:4,Friday:5,Saturday:6},seasonOrder={Spring:0,Fall:1};
    patterns.sort((a,b)=>(seasonOrder[a.season]??9)-(seasonOrder[b.season]??9)||(order[a.day]??9)-(order[b.day]??9)||String(a.field).localeCompare(String(b.field))||String(a.start).localeCompare(String(b.start)));
    let last='';
    const lines=[];
    patterns.forEach(p=>{if(p.season!==last){lines.push(`<b style="display:inline-block;margin-top:5px">${escSI(p.season)} 2026</b>`);last=p.season;}lines.push(`${escSI(p.field)} • ${escSI(pluralDay(p.day))} • ${escSI(p.start)}–${escSI(p.end)} • ${p.hrs.toFixed(1)} hr${Math.abs(p.hrs-1)<0.001?'':'s'}/practice`);});
    return `<div class="small" style="margin-top:10px;line-height:1.65"><b>Practice schedule</b><br>${lines.join('<br>')}</div>`;
  }
